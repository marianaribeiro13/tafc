...
Tracker::Tracker(TGeoManager* geometry, Generator* g, Particle* part, double step, double radius, double height, double distance, double airgap, double althickness, int n_SIPMS,double s_size)
: stepvalue(step), N_photons(0),N_absorbed(0),N_detected(0),N_lost(0),DoubleCross(false)
{
  ...
  auto main_track = geom->GetTrack(0);
...
void Tracker::Propagate_Muon()
{
  ...
  {
    if(CheckDensity_Muon()==0) //Particle is in vacuum
    {
      Muon_Vacuum_Step();
    }
    if(CheckDensity_Muon()==1.023) //Particle is in a scintillator
    {
      scintillator_cross++; //Plus one scintillator crossed
      Muon_Scintillator_Step();
    }
    if(CheckDensity_Muon()==2.7) //Particle is in the aluminium foil
    {
      Muon_Aluminium_Step();
    }
...
void Tracker::Propagate_Photon(int n_start, int n_end) //thread function for photons
{
...
{
  int n_threads = 100;
  int n_photons= n/n_threads;
  cout << "Numero total fotoes: " << n << endl;
  cout << "Numero total fotoes por thread: " << n_photons << endl;
  geom->SetMaxThreads(n_threads);
  for(int i=0; i<n_threads-1; i++)
  {
    navigators_photons.emplace_back(std::thread(&Tracker::Propagate_Photon, this, i*n_photons, i*n_photons+n_photons));
  }
  navigators_photons.emplace_back(std::thread(&Tracker::Propagate_Photon, this, 99*n_photons, n));
  //Join threads (wait for all threads to finish before continuing)
  for (auto &navigator : navigators_photons) {
    navigator.join();
  }
}
...
double Tracker::CheckDensity_Muon()
{
