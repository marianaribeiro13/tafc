...
public:
  ...
  /////Photon Propagators/////
  void Propagate_Photon(int i, int n);
  void Propagate_Photons(int i);
  //void InitializePhotonTrack(int);
  //void Photon_Scintillator_Step(int); //Removed
  void Photon_Scintillator_Reflection_Check(int i);
  // void Photon_Absorbtion(int,double); //Removed
  //void Photon_Vacuum_Step(int); //Removed
  bool Photon_Vacuum_Reflection_Check(int i);
  ...
  vector<Particle*> GetPhotonVector(){return Photons;};
  
  double CheckDensity(); //Move to Geometry
  double CheckDensity_Muon();
